...
public:
    ...
    inline executor_id owner(const uint64_t a)
    {
        return view_map[a].owner;
    }

    inline executor_id author(const uint64_t a)
    {
        return view_map[a].author;
    }

    inline uint64_t parent(void * const c)
    {
        return parent_map[c];
    }

    inline void * child(const uint64_t a)
    {
        return view_map[a].child;
    }
    ...
    std::string to_string(const uint64_t a)
    {
        ...
        s << "(committed=" << committed(a) << " access=" << access_level(a)
                << " owner=" << owner(a) << " author=" << author(a) << " child="
                << child(a) << ")";
        return s.str();
    }
